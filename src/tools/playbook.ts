...
3. **Pick the play (combine when budget allows):**
   - **Conversations play** — create_campaign with 3–8 focused keywords (PRIVATE by default — ask before making it public: public drafts are posted by the network and bill the user per reply). Then run_research (FREE): its expanded keywords feed the next discovery run automatically. Then generate_posts, get_posts, review_posts (dry_run first), regenerate_replies where drafts miss the tone. Manage anytime in the workbench via the campaign's admin_url.
